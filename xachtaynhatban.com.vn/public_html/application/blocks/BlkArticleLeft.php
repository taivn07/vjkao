<?php
class Block_BlkArticleLeft extends Zend_View_Helper_Abstract{

	public function blkArticleLeft($template = 'default'){
		$view  = $this->view;
		...
		if($flagShow == true){
			...
			->where('ac.block_left = ?',1, INTERGER)
			->order('ac.order ASC');
			...
			if (count ( $row ) > 0) {
				require_once (BLOCK_PATH . '/BlkArticleLeft/'.$template.'.php');
			}
		}
	}
}