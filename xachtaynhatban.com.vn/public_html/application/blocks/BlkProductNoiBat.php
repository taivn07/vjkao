<?php
class Block_BlkProductNoiBat extends Zend_View_Helper_Abstract{

	public function blkProductNoiBat($template = 'default'){
		$view  = $this->view;
		...
		if($flagShow == true){
			...
			$select = $db->select()
				...
				->where('p.block_noibat = ?',1,INTERGER)
				->limit(12,0)
				->order('p.order ASC')
				->order('p.id ASC');
			...
			if(count($row) > 0){
				require_once (BLOCK_PATH . '/BlkProductNoiBat/'.$template.'.php');
			}
		}
	}
}