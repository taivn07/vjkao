<?php
class Shopping_AdminItemController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		...
		$tblProducts = new Shopping_Model_Item();
		$this->view->Items = $tblProducts->listItem($this->_arrParam, array('task'=>'admin-list'));

		$this->view->slbCategory = $tblProducts->itemInSelectbox($this->_arrParam);
		
		$tblLanguages = new Default_Model_Language();
		$this->view->slbLanguages = $tblLanguages->itemInSelectbox($this->_arrParam);
		
		$totalItem = $tblProducts->countItem($this->_arrParam, array('task'=>'admin-list'));
	...
	public function addAction(){
		...
		$tblUnits = new Shopping_Model_Units();
		$this->view->slbProductUnits = $tblUnits->itemInSelectbox($this->_arrParam, array('task'=>'get-units'));
		
		$tblManu = new Shopping_Model_Manu();
		$this->view->slbProductManu = $tblManu->itemInSelectbox($this->_arrParam, array('task'=>'get-sources'));
		
		$tblSources = new Shopping_Model_Sources();
		$this->view->slbProductSources = $tblSources->itemInSelectbox($this->_arrParam, array('task'=>'get-sources'));
		...
		if($this->_request->isPost()){
			...
			$validator = new Shopping_Form_ValidateItem($this->_arrParam);
			if($validator->isError() == true){
				$this->view->messageError = $validator->getMessageError();
				$this->view->Item = $validator->getData();
			}else{
				$arrParam = $validator->getData();
				$tblProducts->saveItem($arrParam,array('task'=>'admin-add'));
				$this->_redirect($this->_actionMain);
			}
		}
	}
	...
	public function editAction(){
		...
		$tblProducts = new Shopping_Model_Item();
		$this->view->Item = $tblProducts->getItem($this->_arrParam,array('task'=>'admin-edit'));
		$this->view->slbCategory = $tblProducts->itemInSelectbox($this->_arrParam);
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$arrParam = $validator->getData();
				$tblProducts->saveItem($arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		$this->view->Title = 'Sản phẩm :: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblProducts = new Shopping_Model_Item();
				$tblProducts->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
				if(!empty($this->_arrParam['id'])){
				$tblProducts = new Shopping_Model_Item();
				$tblProducts->deleteItem($this->_arrParam, array('task'=>'admin-delete'));
				$this->_redirect($this->_actionMain . $this->_page);
			}
		}
	}
	...
	public function statusAction(){
		$tblProducts = new Shopping_Model_Item();
		$tblProducts->changeStatus($this->_arrParam);
		$this->_redirect($this->_actionMain . $this->_page);
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblProducts = new Shopping_Model_Item();
			$tblProducts->sortItem($this->_arrParam, array('task'=>'admin-sort'));
			$this->_redirect($this->_actionMain . $this->_page);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
