...
class Shopping_Model_Category extends Zend_Db_Table{
	...
	public function countItem($arrParam = null, $options = null){
		...
		$ssFilter  = $arrParam['ssFilter'];
		$select = $db->select()
		->from('product_category AS pc', array('COUNT(pc.id) AS totalItem'));
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-list'){
			$select = $db->select()
			->from('product_category AS pc',array('id','name','parents','status','order','created_by','lang_code'))
		...
		if($options['task'] == 'public-subcategories'){
			$select = $db->select()
			->from('product_category AS pc',array('id','name','alias','parents','picture'))
			->order('pc.order ASC')
			->order('pc.id ASC');
