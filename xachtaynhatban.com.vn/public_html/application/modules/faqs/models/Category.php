<?php
class Faqs_Model_Category extends Zend_Db_Table{
	
	protected $_name = 'faqs_category';
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-list'){
			$select = $db->select()
			->from('faqs_category AS fc',array('id','name','parents','status','order','created_by','lang_code'))
			->joinLeft('users AS u', 'u.id = fc.created_by','user_name')
			->order('fc.order ASC')
			->order('fc.id ASC');
			
			if(!empty($ssFilter['keywords'])){
				$keywords = '%' . $ssFilter['keywords'] . '%';
				$select->where('fc.name LIKE ?', $keywords, STRING);
			}
