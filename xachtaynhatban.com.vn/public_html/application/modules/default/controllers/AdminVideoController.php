<?php
class AdminVideoController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		$this->view->Title = 'Video :: Danh sách';
		$this->view->headTitle($this->view->Title, true);
		
		$tblVideo = new Default_Model_Video();
		$this->view->Items = $tblVideo->listItem($this->_arrParam, array('task'=>'admin-list'));
		
		$tblLanguages = new Default_Model_Language();
		$this->view->slbLanguages = $tblLanguages->itemInSelectbox($this->_arrParam);
		
		$totalItem = $tblVideo->countItem($this->_arrParam, array('task'=>'admin-list'));
	...
	public function addAction(){
		$this->view->Title = 'Video :: Thêm mới';
		$this->view->headTitle($this->view->Title, true);
		
		$tblVideo = new Default_Model_Video();
		$this->view->slbCategory = $tblVideo->itemInSelectbox($this->_arrParam);
		
		if($this->_request->isPost()){
				
			$validator = new Default_Form_ValidateVideo($this->_arrParam);
			if($validator->isError() == true){
				$this->view->messageError = $validator->getMessageError();
				$this->view->Item = $validator->getData();
			}else{
				$arrParam = $validator->getData();
				$tblVideo->saveItem($arrParam,array('task'=>'admin-add'));
				$this->_redirect($this->_actionMain);
			}
		}
	}
	...
	public function infoAction(){
		$this->view->Title = 'Video :: Xem thông tin';
		$this->view->headTitle($this->view->Title, true);
		$tblVideo = new Default_Model_Video();
		$this->view->Item = $tblVideo->getItem($this->_arrParam,array('task'=>'admin-info'));
		
	}
	
	public function editAction(){
		$this->view->Title = 'Video :: Sửa';
		$this->view->headTitle($this->view->Title, true);
		
		$tblVideo = new Default_Model_Video();
		$this->view->Item = $tblVideo->getItem($this->_arrParam,array('task'=>'admin-edit'));
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$arrParam = $validator->getData();
				$tblVideo->saveItem($arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		$this->view->Title = 'Video :: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblVideo = new Default_Model_Video();
				$tblVideo->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
				if(!empty($this->_arrParam['id'])){
				$tblVideo = new Default_Model_Video();
				$tblVideo->deleteItem($this->_arrParam, array('task'=>'admin-delete'));
				$this->_redirect($this->_actionMain . $this->_page);
			}
		}
	}
	...
	public function statusAction(){
		$tblVideo = new Default_Model_Video();
		$tblVideo->changeStatus($this->_arrParam);
		$this->_redirect($this->_actionMain . $this->_page);
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblVideo = new Default_Model_Video();
			$tblVideo->sortItem($this->_arrParam, array('task'=>'admin-sort'));
			$this->_redirect($this->_actionMain . $this->_page);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
