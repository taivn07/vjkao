<?php
class Gallery_AdminAlbumController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		$this->view->Title = 'Thư viện ảnh :: Album :: Danh sách';
		$this->view->headTitle($this->view->Title, true);
		
		$tblAlbum = new Gallery_Model_Album();
		$this->view->Items = $tblAlbum->listItem($this->_arrParam, array('task'=>'admin-list'));

		$this->view->slbCategory = $tblAlbum->itemInSelectbox($this->_arrParam);
		
		$tblLanguages = new Default_Model_Language();
		$this->view->slbLanguages = $tblLanguages->itemInSelectbox($this->_arrParam);
		
		$totalItem = $tblAlbum->countItem($this->_arrParam, array('task'=>'admin-list'));
	...
	public function addAction(){
		$this->view->Title = 'Thư viện ảnh :: Album :: Thêm mới';
		$this->view->headTitle($this->view->Title, true);
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$tblAlbum->saveItem($arrParam,array('task'=>'admin-add'));
				$this->_redirect($this->_actionMain);
			}
		}
	}
	...
	public function infoAction(){
		$this->view->Title = 'Thư viện ảnh :: Album :: Xem thông tin';
		$this->view->headTitle($this->view->Title, true);
		$tblAlbum = new Gallery_Model_Album();
		$this->view->Item = $tblAlbum->getItem($this->_arrParam,array('task'=>'admin-info'));
	}
	
	public function editAction(){
		$this->view->Title = 'Thư viện ảnh :: Album :: Sửa';
		$this->view->headTitle($this->view->Title, true);
		
		$tblAlbum = new Gallery_Model_Album();
		$this->view->Item = $tblAlbum->getItem($this->_arrParam,array('task'=>'admin-edit'));
		...
		if($this->_request->isPost()){
			...
			if($validator->isError() == true){
				...
				$arrParam = $validator->getData();
				$tblAlbum->saveItem($arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		$this->view->Title = 'Thư viện ảnh :: Album :: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblAlbum = new Gallery_Model_Album();
				$tblAlbum->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
				if(!empty($this->_arrParam['id'])){
				$tblAlbum = new Gallery_Model_Album();
				$tblAlbum->deleteItem($this->_arrParam, array('task'=>'admin-delete'));
				$this->_redirect($this->_actionMain . $this->_page);
			}
		}
	}
	...
	public function statusAction(){
		$tblAlbum = new Gallery_Model_Album();
		$tblAlbum->changeStatus($this->_arrParam);
		$this->_redirect($this->_actionMain . $this->_page);
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblAlbum = new Gallery_Model_Album();
			$tblAlbum->sortItem($this->_arrParam, array('task'=>'admin-sort'));
			$this->_redirect($this->_actionMain . $this->_page);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
