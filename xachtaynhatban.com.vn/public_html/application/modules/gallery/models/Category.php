<?php
class Gallery_Model_Category extends Zend_Db_Table{
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-list'){
			$select = $db->select()
			->from('gallery_category AS gc',array('id','name','parents','status','order','created_by','lang_code'))
			->joinLeft('users AS u', 'u.id = gc.created_by','user_name')
			->order('gc.order ASC')
			->order('gc.id ASC');
			
			if(!empty($ssFilter['keywords'])){
				$keywords = '%' . $ssFilter['keywords'] . '%';
				$select->where('gc.name LIKE ?', $keywords, STRING);
			}
