...
//Popup
function jquery_popup(p_width, p_height, p_content) {
	$('.box_overlay').remove();
	$('.block_popup').remove();
	var str_html = 	'<div class="box_overlay"></div>'+
	...
	if(p_width == 0) {
		p_width = $('.block_popup .block_content').width();
	}
	if(p_height == 0) {
		p_height = $('.block_popup .block_content').height();
	}
	...
	$('.block_popup .block_content').html(p_content);
	
	$('.popup_close,.box_overlay,.popup_dong').click(function(){
		close_popup();
	});
}
...
function popupCart(idAdd){
	var str_html = 	'<div class="box_overlay"><div class="dangtai"></div></div>';
	$('body').append(str_html);
	var soluong = 1;
	if($("#txtnum").length) { // 0 == false; >0 == true
		soluong = $("#txtnum").val();
	}
	var p_content = '';
	$.ajax({
		type: "GET",
		url: base_url + '/shopping/public/add-item/id/' + idAdd + '/soluong/' + soluong,
		success: function(html){
			$.ajax({
				type: "GET",
				url: base_url + '/shopping/public/popup-cart/id/' + idAdd,
				success: function(html){
					jquery_popup(830, 550, html);
			    }
			});
	    }
	});
...
function loadTotalCart(){
	$('#total_Cart').load(base_url + '/shopping/public/total');
}

function loadListCart(){
	$('#danhsach_giohang').load(base_url + '/shopping/public/danhsach');
}

function xoaCart(idItem){
	$('#danhsach_giohang').load(base_url + '/shopping/public/danhsach/delete/' + idItem, loadTotalCart());
}
