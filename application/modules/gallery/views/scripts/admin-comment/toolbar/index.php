<?php
	...
	// 3. Thêm mới - link
	$linkAddNew = $this->baseUrl ( $this->currentController . '/add' . $page  );
	...
	if($this->arrParam['action'] == 'add'){
		$linkSaveItem = $this->baseUrl ( $this->currentController . '/add' . $page );
	}else if ($this->arrParam['action'] == 'add-reply'){
		$linkSaveItem = $this->baseUrl ( $this->currentController . '/add-reply/id/' . $this->arrParam['id'] . $page );
	}else if ($this->arrParam['action'] == 'edit-reply'){
		$linkSaveItem = $this->baseUrl ( $this->currentController . '/edit-reply/id/' . $this->arrParam['id'] . '/reply/' . $this->arrParam['reply'] . $page );
	}else{
	...
	if($this->arrParam['action'] == 'delete-reply')
		$linkAccept = $this->baseUrl ( $this->currentController . '/delete-reply/id/' . $this->arrParam['id'] . '/reply/' . $this->arrParam['reply'] . $page);
	$btnAccept = $this->cmsButton ( 'Đồng ý', $linkAccept, $this->imgUrl . '/toolbar/icon-32-apply.png', 'submit' );
	
	$linkDelete = $this->baseUrl ( $this->currentController . '/delete/id/' . $this->arrParam['id'] . $page );
	$btnDelete = $this->cmsButton ( 'Xóa', $linkDelete, $this->imgUrl . '/toolbar/icon-32-delete.png', 'link' );
	
	$divider = '<div class="divider"> </div>';
	
	switch ($this->arrParam['action']){
		
		case 'index': $strBtn = $btnDeleteItems;
					 			break;
					 		
		case 'edit': $strBtn = $btnSaveItem . ' ' . $btnCancel;
				 				break;
		
		case 'edit-reply': $strBtn = $btnSaveItem . ' ' . $btnCancel;
				 				break;
			
		case 'add': $strBtn = $btnSaveItem . ' ' . $btnCancel;
				 				break;
				 				
		case 'add-reply': $strBtn = $btnSaveItem . ' ' . $btnCancel;
				 				break;
				 				
		case 'delete': $strBtn = $btnAccept . ' ' . $btnCancel;
								break;
								
		case 'delete-reply': $strBtn = $btnAccept . ' ' . $btnCancel;
								break;
								
		case 'info': $strBtn =  $btnDelete . ' ' . $btnBack;
								break;
