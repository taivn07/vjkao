<?php
	...
	$btnActiveItems = $this->cmsButton ( 'Bật', $linkActiveItems, $this->imgUrl . '/toolbar/icon-32-publish.png', 'submit' );
	
	// 2. Inactive Items - submit
	$linkInactiveItems = $this->baseUrl ( $this->currentController . '/status/type/0' . $page );
	$btnInactiveItems = $this->cmsButton ( 'Tắt', $linkInactiveItems, $this->imgUrl . '/toolbar/icon-32-unpublish.png', 'submit' );
	...
	$btnSortItem = $this->cmsButton ( 'Sắp xếp', $linkSortItem, $this->imgUrl . '/toolbar/icon-32-sort.png', 'submit' );
	
	// 5. Xóa nhiều - submit
	$linkDeleteItems = $this->baseUrl ( $this->currentController . '/delete/type/multi-delete' . $page);
	$btnDeleteItems = $this->cmsButton ( 'Xóa nhiều', $linkDeleteItems, $this->imgUrl . '/toolbar/icon-32-delete.png', 'submit' );
	
	// 6. Luu = submit
	if($this->arrParam['action'] == 'add'){
		$linkSaveItem = $this->baseUrl ( $this->currentController . '/add' );
	}else{
		$linkSaveItem = $this->baseUrl ( $this->currentController . '/edit/id/' . $admin_info['id']);
	}
	$btnSaveItem = $this->cmsButton ( 'Lưu', $linkSaveItem, $this->imgUrl . '/toolbar/icon-32-save.png', 'submit' );
	
	// 7. Cancel - Link
	$linkCancel = $this->baseUrl ( '/default/admin' );
	...
	$btnSavePassword = $this->baseUrl ( $this->currentController . '/password/id/' . $admin_info['id']);
	$btnSavePassword = $this->cmsButton ( 'Lưu', $btnSavePassword, $this->imgUrl . '/toolbar/icon-32-save.png', 'submit' );
	
	switch ($this->arrParam['action']){
		
		case 'index': $strBtn = $btnCancel;
					 			break;
					 		
		case 'edit': $strBtn = $btnSaveItem . ' ' . $btnCancel;
				 				break;
		
		case 'password': $strBtn = $btnSavePassword . ' ' . $btnCancel;
				 				break;
	...
	//Icon toolbar
	$IconToolbar = 'icon-48-user-profile';
?>
