...
class AdminContactController extends Zendvn_Controller_Action{
	...
	public function addAction(){
		...
		$tblContact = new Article_Model_Item();
		$this->view->slbCategory = $tblContact->itemInSelectbox($this->_arrParam);
	...
	public function infoAction(){
		$this->view->Title = 'Liên hệ :: Xem thông tin';
		$this->view->headTitle($this->view->Title, true);
		$tblContact = new Default_Model_Contact();
		$this->view->Item = $tblContact->getItem($this->_arrParam,array('task'=>'admin-info'));
		
	}
	
	public function editAction(){
		/* $this->view->Title = 'Liên hệ :: Sửa';
		$this->view->headTitle($this->view->Title, true);
	...
	public function deleteAction(){
		$this->view->Title = 'Liên hệ :: Xóa';
		$this->view->headTitle($this->view->Title, true);
		if($this->_request->isPost()){
			if(($this->_arrParam['type'] == 'multi-delete') && ($this->_arrParam['task'] == 'ok')){
				$tblContact = new Default_Model_Contact();
				$tblContact->deleteItem($this->_arrParam, array('task'=>'admin-delete-muti'));
				$this->_redirect($this->_actionMain . $this->_page);
			}else
