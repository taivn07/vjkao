	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'public-category'){
			if(!empty($arrParam['cid'])){
				...
				$paginator = $arrParam['paginator'];
				$select = $db->select()
				->from('products AS p',array('id','code','name','alias','synopsis','picture','thumb','cat_id','price','selloff','units_money','khuyenmai'))
		...
		if($options['task'] == 'public-block'){
			...
			$paginator = $arrParam['paginator'];
			$select = $db->select()
			->from('products AS p',array('id','name','alias','picture','thumb','cat_id','price','selloff','units_money','khuyenmai'))
			->join('product_category AS pc', 'pc.id = p.cat_id',array('name AS category_name', 'pc.alias AS category_alias'))
			->where('p.status = ?',1,INTERGER)
			->order('p.order ASC')
			->order('p.id DESC');
